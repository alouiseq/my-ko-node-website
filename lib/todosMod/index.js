...
// Basic Synchronous Authentication
var auth = express.basicAuth('sexy', 'beast');
...
// Get skills and rankings
app.get('/skillsRanks', function(req, res) {
  var db = req.db;
  db.collection('skills').find().toArray(function(err, items) {
      res.send(items);
  });
});
...
app.post('/storeSkills/', function(req, res) {
  var db = req.db,
      keys = Object.keys(req.body),
      parsedData = JSON.parse(keys[0]),
      skill = parsedData.skill,
      taskList = parsedData.taskList,
      exp = parsedData.exp,
  ...
  db.collection('skills').update(
    { skill: skill },
    { 
      $set: { recent: taskList, exp: exp, level: level, ranking: rank } 
    },  
    function(err, result) {
      if (err) {
	res.send({msg: JSON.stringify(err)});
